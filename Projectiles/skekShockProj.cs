...
{
	class skekShockProj : ModProjectile
	{
		...
		public override void AI()
		{
			...
			dust3.shader = GameShaders.Armor.GetSecondaryShader(99, Main.LocalPlayer);
			if (Main.rand.NextFloat() < 0.15f)
			{
				Dust dust4;
				dust4 = Main.dust[Terraria.Dust.NewDust(projectile.position, 0, 0, 6, 0f, 0f, 0, new Color(255, 255, 255), 1f)];
				dust4.shader = GameShaders.Armor.GetSecondaryShader(99, Main.LocalPlayer);
			}
			if (Main.rand.NextFloat() < 0.2105263f)
			{
				...
				dust5.shader = GameShaders.Armor.GetSecondaryShader(99, Main.LocalPlayer);
			}
		}
		public override void Kill(int timeLeft)
		{
			Projectile.NewProjectile(projectile.position.X, projectile.position.Y, 0, 0, mod.ProjectileType("skekShockExplosion"), (int)(projectile.damage * 1.3), 0f, projectile.owner, 0f, 0f);
		}
