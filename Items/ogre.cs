...
namespace bl3guns.Items
{
    public class ogre : ModItem
    {
        public override void SetStaticDefaults()
        {
            ...
            Tooltip.SetDefault("[Vladof]\nFires explosive bolts\n15% cnance not to consume ammo\n'It's all ogre now.'");
        }
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            Projectile.NewProjectile(position.X, position.Y, speedX, speedY, mod.ProjectileType("ogreProj"), damage, knockBack, player.whoAmI, speedX, speedY);
            Projectile.NewProjectile(position.X, position.Y, speedX, speedY, mod.ProjectileType("ogreProj"), damage, knockBack, player.whoAmI, speedX, speedY);
            Main.PlaySound(mod.GetSoundSlot(SoundType.Item, "Sounds/Item/smg"), (int)player.Center.X, (int)player.Center.Y);
